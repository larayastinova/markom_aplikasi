...
<section class="content">
	<div class="card">
		<div class="card-header">
            ...
            <button type="button" class="btn btn-tool" data-toggle="modal" data-target="#create_role">
		...
		<div class="card-body">
			<table class="table">
				<tr>
					<td>
						<select id="search_role_code" name="search_role_code" class="form-control">
							<option>-Select Role Code-</option>
						</select>
					</td>
					<td>
						<select id="search_role_name" name="search_role_name" class="form-control">
	...
	<div class="modal fade" id="create_role" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="card-header">
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
					<h4 class="card-title" id="myModalLabel">Add Role</h4>
				</div>
				...
				<div class="modal-body">
					<form id="addForm" data-toggle="validator" action="#" method="POST">
						<div class="form-group">
							...
							<input type="text" class="form-control" name="role_code" id="role_code" placeholder="Auto Generate" readonly />
	...
	<div class="modal fade" id="change_role" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="card-header">
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
					<h4 class="card-title" id="myModalLabel">Edit role</h4>
				</div>
				<div id="form-edit">
		...
		$(document).on('click','#btn_add',function(){
			...
			var role_name = $('#role_name').val();
				...
				$.ajax({
					type: 'POST',
					url: 'task/role.php',
					data: {'action':'save_role','username':username,'nama':nama,'m_role_id':m_role_id,'search_role_code':search_role_code,'search_role_name':search_role_name,'created_by':created_by,'datepicker':datepicker,'role_name':role_name,'description':description},
		...
		$(document).on('click','#btn_edit_data',function(){
			...
			//if(error == 0){
				$.ajax({
					type: 'POST',
					url: 'task/role.php',
					data: {'action':'edit_role','username':username,'m_role_id':m_role_id,'id_edit':id_edit,'description_edit':description_edit,'role_edit':role_edit},
					success: function(data){
						sp = data.split('~');
						if(sp[1] == 'Sukses'){
							$('#modal').modal('toggle');
